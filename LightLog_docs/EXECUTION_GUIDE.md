...
```
개발 서버가 켜진 상태에서, 안드로이드/iOS 기기 또는 에뮬레이터에 설치된 **Development Build 앱 (LightLog_FE)** 을 직접 실행합니다. 앱이 자동으로 개발 서버에 연결되며, 코드 수정 시 실시간으로 변경 내용이 반영됩니다.

**이제 Expo Go 앱은 사용하지 않습니다.**

### 2. 웹 브라우저에서 실행

로컬 PC의 웹 브라우저에서 앱을 빠르게 확인하려면 다음 명령어를 사용합니다.
```bash
npm run web
```
*이 명령어는 자동으로 웹 브라우저를 열고 앱을 실행합니다.*
